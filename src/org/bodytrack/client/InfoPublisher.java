...
public final class InfoPublisher {
	...
	public static native void setUpWrappers() /*-{
		...
		///		must be numbers, with the max value greater than the min value.
		///		If this key is not present, a default range will be assigned
		function DateAxis(placeholder, orientation, range) {
			if (placeholder === undefined || orientation === undefined) {
				throw 'Must pass in placeholder and orientation';
			}
			...
			this.__backingAxis = function() {
				...
				return @org.bodytrack.client.TimeGraphAxis::new(Ljava/lang/String;DDLorg/bodytrack/client/Basis;DZ)(placeholder, axisMin, axisMax, basis, axisWidth, isXAxis);
			}();
			this.getMin = function() {
				return this.__backingAxis.@org.bodytrack.client.TimeGraphAxis::getMin()();
			};
			this.getMax = function() {
				return this.__backingAxis.@org.bodytrack.client.TimeGraphAxis::getMax()();
			};
			this.setRange = function(min, max) {
				// Exceptions for illegal min and max values are handled in Java
				this.__backingAxis.@org.bodytrack.client.TimeGraphAxis::replaceBounds(DD)(min, max);
			};
		}
	}-*/;
}
